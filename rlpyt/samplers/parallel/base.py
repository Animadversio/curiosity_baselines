...
class ParallelSamplerBase(BaseSampler):
    ...
    def initialize(
        ...
        for i, w_kwargs in enumerate(workers_kwargs):
            if i != 0:
                common_kwargs['record_freq'] = 0 # only record first worker
            self.workers.append(mp.Process(target=target, kwargs=dict(common_kwargs=common_kwargs, worker_kwargs=w_kwargs)))
